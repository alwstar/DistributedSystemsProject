...
server_name = ''.join(random.choices(string.ascii_uppercase, k=5))
...
def receive_leader():
    ...
    while True:
        ...
        print(f'LEADER IS: {multicast_data.LEADER} (I am {server_name})')
...
def bind_server_sock():
    try:
        ...
        print(f'Server started on IP {server_data.SERVER_IP} and PORT {ports.SERVER_PORT_FOR_CLIENTS} (I am {server_name})')
...
def update_server_list(new_list):
    if len(multicast_data.SERVER_LIST) == 0:
        ...
        if multicast_data.LEADER != server_data.SERVER_IP:
            ...
            print(f'My server list is empty, the new leader is me {server_data.SERVER_IP} (I am {server_name})')
