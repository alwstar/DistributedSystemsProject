...
def start_heartbeat():
    ...
    while server_data.HEARTBEAT_RUNNING:
        ...
        broadcast_data.SERVER_LIST = list(set(broadcast_data.SERVER_LIST))
        for x in range(len(broadcast_data.SERVER_LIST)):
            time.sleep(1)
            if x > len(broadcast_data.SERVER_LIST):
            ...
            if server_data.isReplicaUpdated == True:
                ...
                break
            time.sleep(1)
            print(broadcast_data.SERVER_LIST)
            try:
                ip = broadcast_data.SERVER_LIST[x]
            except IndexError:
                broadcast_data.SERVER_LIST = list(set(broadcast_data.SERVER_LIST))
                server.send_server_list()
                break
            ...
            try:
                ...
                del broadcast_data.SERVER_LIST[failed_server]
            finally:
                s.close()
        if failed_server >= 0:
            new_server_list = broadcast_data.SERVER_LIST
...
def restart_heartbeat():
    if server_data.isReplicaUpdated:
        ...
        if server_data.LEADER_CRASH:
            ...
            leader_election.start_leader_election(broadcast_data.SERVER_LIST, server_data.SERVER_IP)
