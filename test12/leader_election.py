...
def sendnewLeaderMessage():
    if broadcast_data.LEADER == server_data.SERVER_IP:
        # New Leader IP
        #broadcast_data.LEADER = get_neighbour(form_ring(broadcast_data.SERVER_LIST), broadcast_data.LEADER)
        msg = server_data.SERVER_IP

        for x in range(len(broadcast_data.SERVER_LIST)):
            ip = broadcast_data.SERVER_LIST[x]  # serverlist consists a tuple of a tuple and a boolean. The inside tuple are the connection details host ip and port
...
def listenforNewLeaderMessage():
    ...
    while True:
        ...
        for i in range(len(broadcast_data.SERVER_LIST)):  # search for the leader IP in the serverlist
            ip = broadcast_data.SERVER_LIST[i]  # serverlist consists a tuple of a tuple and a boolean. The inside tuple are the connection details host ip and port
        ...
        broadcast_data.LEADER = newleader_ip
