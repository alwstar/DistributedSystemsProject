...
def send_leader():
    if broadcast_data.LEADER == server_data.SERVER_IP and len(broadcast_data.SERVER_LIST) > 0:
        for i in range(len(broadcast_data.SERVER_LIST)):
            replica = broadcast_data.SERVER_LIST[i]
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(1)
            try:
                sock.connect((replica, ports.LEADER_NOTIFICATION_PORT))
                leader_message = pickle.dumps(broadcast_data.LEADER)
                sock.send(leader_message)
                logging.info(f'Leader {broadcast_data.LEADER} is updating the leader parameter for {replica}')
...
def receive_leader():
    ...
    while True:
        ...
        broadcast_data.LEADER = leader
        print(f'LEADER IS: {broadcast_data.LEADER}')

# sends server List to replica servers over TCP
def send_server_list():
    if broadcast_data.LEADER == server_data.SERVER_IP and len(broadcast_data.SERVER_LIST) > 0:
        for i in range(len(broadcast_data.SERVER_LIST)):
            if broadcast_data.SERVER_LIST[i] != server_data.SERVER_IP:
                replica = broadcast_data.SERVER_LIST[i]
                ip = replica
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.settimeout(3)
                ...
                try:
                    ...
                    updated_list = pickle.dumps(broadcast_data.SERVER_LIST)
    ...
    while True:
        ...
        for i in range(server_list_len):
            ...
            if ip == server_data.SERVER_IP:
                ...
                broadcast_data.SERVER_LIST = new_list
                print(f'NEW SERVER LIST {broadcast_data.SERVER_LIST}')
                sleep(0.5)
                update_server_list(broadcast_data.SERVER_LIST)


# sends client messages to other clients over a TCP Socket
def send_new_client_message(ip, msg):
    if broadcast_data.LEADER == server_data.SERVER_IP and len(broadcast_data.CLIENT_LIST) > 0:
        for i in range(len(broadcast_data.CLIENT_LIST)):
            client = broadcast_data.CLIENT_LIST[i]
            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            sock.settimeout(3)
            if ip != client:
                try:
    ...
    while True:
        try:
            ...
            if data.decode('utf-8') != "":
                ...
                broadcast_data.CLIENT_MESSAGES.append(f'{server_data.SERVER_IP}: new Message from {address[0]}: {data.decode("utf-8")}')
                send_new_client_message(address[0], data.decode('utf-8'))
                #print(f'CLIENT LIST {broadcast_data.CLIENT_LIST}')
        except Exception as err:
            print(err)
            break
...
def send_client_list():
    if broadcast_data.LEADER == server_data.SERVER_IP and len(broadcast_data.SERVER_LIST) > 0:
        for i in range(len(broadcast_data.SERVER_LIST)):
            if broadcast_data.SERVER_LIST[i] != server_data.SERVER_IP:
                ip = broadcast_data.SERVER_LIST[i]
                sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                sock.settimeout(1)
                try:
                    sock.connect((ip, ports.CLIENT_LIST_UPDATE_PORT))

                    updated_list = pickle.dumps(broadcast_data.CLIENT_LIST)
                    sock.send(updated_list)
...
def receive_client_list():
    ...
    while True:
        ...
        broadcast_data.CLIENT_LIST = new_list
        print(f'NEW CLIENT LIST {broadcast_data.CLIENT_LIST}')


if __name__ == '__main__':
    broadcastSender = broadcast_sender.requestToBroadcast()

    # checking if broadcast receiver is available
    if not broadcastSender:
        broadcast_data.LEADER = server_data.SERVER_IP
        server_data.LEADER_CRASH = False
        server_data.LEADER_AVAILABLE = True

    thread_helper.newThread(broadcast_receiver.start_receiver, ())
    ...
    while True:
        try:
            if broadcast_data.LEADER and broadcast_data.network_state:
                broadcast_sender.requestToBroadcast()
                broadcast_data.network_state = False

            elif broadcast_data.LEADER != server_data.SERVER_IP and broadcast_data.network_state:
                broadcast_data.network_state = False
